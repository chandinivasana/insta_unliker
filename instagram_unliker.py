...
class InstagramUnliker: 
    ...
    def add_account(self):
        ...
        print("-" * 40)
        print("1. Login with Username/Password (Often blocked)")
        print("2. Login with Session ID (Recommended - Bypasses bot detection)")
        
        mode = input(f"\n{ConsoleColors.BOLD}Select login mode (1/2): {ConsoleColors.RESET}").strip()
        
        if mode == "2":
            username = input(f"{ConsoleColors.BOLD}Username: {ConsoleColors.RESET}").strip()
            session_id = input(f"{ConsoleColors.BOLD}Session ID: {ConsoleColors.RESET}").strip()
            if not username or not session_id:
                print(f"{ConsoleColors.RED}Username and Session ID are required{ConsoleColors.RESET}")
                return
            password = f"SESSION_ID:{session_id}"
        else:
            username = input(f"{ConsoleColors.BOLD}Username: {ConsoleColors.RESET}").strip()
            password = input(f"{ConsoleColors.BOLD}Password: {ConsoleColors.RESET}").strip()
        
        if not username or not password:
            print(f"{ConsoleColors.RED}Username and credentials are required{ConsoleColors.RESET}")
            return
    ...
    def unlike_posts(self, username: str):
        ...
        try:
            ...
            try:
                ...
                print(f"{ConsoleColors.BLUE}[*] Attempting to login as @{account_data['username']}...{ConsoleColors.RESET}")
                client = Web(account_data['username'], account_data['password'])
                
                # Check if we are actually logged in by trying to get private info
                account = client.private_info()
                if account:
                    print(f"{ConsoleColors.GREEN}✓ Successfully logged in as: {ConsoleColors.CYAN}{account.username}{ConsoleColors.RESET}")
                else:
                    raise Exception("Login appeared to succeed but could not retrieve account info. You might be shadowbanned or flagged.")
                    
            except Exception as e:
                error_str = str(e)
                error_msg = f"Login failed: {error_str}"
                logging.error(error_msg)
                print(f"{ConsoleColors.RED}[✗] {error_msg}")
                
                if "two_factor_required" in error_str.lower():
                    print(f"{ConsoleColors.YELLOW}→ Two-Factor Authentication is required for this account.{ConsoleColors.RESET}")
                elif "checkpoint_required" in error_str.lower():
                    print(f"{ConsoleColors.YELLOW}→ Instagram is asking for a security check. Please log in via a browser first.{ConsoleColors.RESET}")
                
                print(f"→ Troubleshooting: 1. Confirm 'This Was Me' on your phone. 2. Check if password is correct. 3. Disable 2FA temporarily if it continues to fail.{ConsoleColors.RESET}")
                return
